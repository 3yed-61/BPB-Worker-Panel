...
async function renderHomePage (env, hostName, fragConfigs) {
	...
	<head>
        ...
        <title>3yedPanel ${panelVersion}</title>
	...
	<body>
		<h1>3λΞĐPanel <span style="font-size: smaller;">${panelVersion}</span> ♨️</h1>
...
async function buildWorkerLessConfig(remoteDNS, localDNS, lengthMin,  lengthMax,  intervalMin,  intervalMax, blockAds, bypassIran, blockPorn, bypassLAN, bypassChina, blockUDP443) {
    ...
    fragConfig.remarks  = '♨️3yed F - WorkerLess ⭐'
...
async function getFragmentConfigs(env, hostName) {
    ...
    for (let i = 0; i < protocolNo; i++) {
        for (let portIndex in ports.filter(port => defaultHttpsPorts.includes(port))) {
            ...
            for (let index in Addresses) {
                ...
                if (vlessConfigs && i === 0) {
                    remark = generateRemark(proxyIndex, port, addr, cleanIPs, '3yed', 'F');
                    outbound = buildXrayVLESSOutbound('proxy', addr, port, userID, hostName, proxyIP);
                }
                
                if (trojanConfigs && !outbound) {
                    remark = generateRemark(proxyIndex, port, addr, cleanIPs, '3yed', 'F', addr);
    ...
    bestPing.remarks = '♨️3yed F - Best Ping 💥';
    ...
    bestFragment.remarks = '♨️3yed F - Best Fragment 😎';
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? '♨️3yed- Warp Pro Best Ping 🚀' : '♨️3yed- Warp Best Ping 🚀';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `♨️3yed- Warp Pro ${index + 1} 🇮🇷` : `♨️3yed- Warp ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `♨️3yed- WoW Pro ${index/2 + 1} 🌍` : `♨️3yed- WoW ${index/2 + 1} 🌍`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? '♨️3yed- WoW Pro Best Ping 🚀' : '♨️3yed- WoW Best Ping 🚀';
    ...
    for (let i = 0; i < protocolsNo && !isWarp; i++) {
        ports.forEach(port => {
            totalAddresses.forEach((addr, index) => {
                ...
                if (vlessConfigs && i === 0) {
                    remark = generateRemark(proxyIndex, port, addr, cleanIPs, '3yed', configType).replace(' : ', ' - ');
    ...
    for (let i = 0; i < protocolsNo && !isWarp; i++) {
        ports.filter(port => (defaultHttpsPorts.includes(port) && isFragment) || !isFragment).forEach(port => {
            totalAddresses.forEach((addr, index) => {
                ...
                if (vlessConfigs && i === 0) {
                    remark = generateRemark(proxyIndex, port, addr, cleanIPs, '3yed', configType);
                    path = `/${getRandomPath(16)}${proxyIP ? `/${btoa(proxyIP)}` : ''}`;
...
async function getNormalConfigs(env, hostName, client) {
    ...
    ports.forEach(port => {
        totalAddresses.forEach((addr, index) => {
            ...
            const configIndex = isCustomAddr ? index - Addresses.length + 1 : index;
            const vlessRemark = encodeURIComponent(generateRemark(proxyIndex, port, addr, cleanIPs, '3yed', configType));
            const trojanRemark = encodeURIComponent(generateRemark(proxyIndex + totalAddresses.length * ports.length, port, addr, cleanIPs, '3yed', configType));
