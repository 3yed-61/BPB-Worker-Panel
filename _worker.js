...
let trojanPassword = `trojan-3yed`;
...
function generateRemark(index, port, protocol, fragType) {
    ...
    switch (index) {
        case 0:
        case 1:
            remark = `♨️ ${protocol}${type} - Domain ${index + 1} : ${port}`;
            break;
        case 2:
        case 3:
            remark = `♨️ ${protocol}${type} - IPv4 ${index - 1} : ${port}`;
            break;
        case 4:
        case 5:
            remark = `♨️ ${protocol}${type} - IPv6 ${index - 3} : ${port}`;
            break;
        default:
            remark = `♨️ ${protocol}${type} - Clean IP ${index - 5} : ${port}`;
            break;
    }
            ...
            <tr>
                <td>
                    ${config.address === 'Best-Ping' 
                        ? `<div  style="justify-content: center;"><span><b>♨️ 3λΞĐ F - Best-Ping 💥</b></span></div>` 
                        : config.address === 'WorkerLess'
                            ? `<div  style="justify-content: center;"><span><b>♨️ 3λΞĐ F - WorkerLess ⭐</b></span></div>`
                            : config.address === 'Best-Fragment'
                                ? `<div  style="justify-content: center;"><span><b>♨️ 3λΞĐ F - Best-Fragment 😎</b></span></div>`
                                : config.address
                    }
                </td>
                <td>
    ...
    <body>
        <div id="error-container">
            ...
            <div id="error-message">
                <h2>${message} ${refer 
                    ? 'Please try again or refer to <a href="https://github.com/bia-pain-bache/3YED-Worker-Panel/blob/main/README.md">documents</a>' 
                    : ''}
                </h2>
...
async function getClashConfig (env, hostName, isWarp) {
    ...
    let config = {
        ...
        "proxy-groups": [
            ...
            {
                "name": isWarp ? `♨️ Warp Best Ping 🚀`: `♨️ Best Ping 💥`,
                "type": "url-test",
